...
- 原生采集：自动探测 MJPEG `4000×1200@30 FPS` 或 `3840×1080@30 FPS`
- 单眼分辨率：`world intelligent` 为 `1920×1200`，普通 SBS 为 `1920×1080`
- 当前实测棋盘：`5×8` 内角点（6×9 个方格）
- 默认采集目标：32 组

标定时必须让左右眼同时看到完整棋盘。方格实际边长决定平移向量、基线和深度的物理尺度，必须用尺测量后传给 `--square-mm`。

### World Intelligent 相机识别
...
```text
frame[:, 0:160]       码带（丢弃）
frame[:, 160:2080]    左眼 1920×1200
frame[:, 2080:4000]   右眼 1920×1200
```

名称和分辨率匹配但码带检测失败时，平台会停止采集，不会错误地执行普通等分。标定和实际运行必须使用相同裁剪范围，否则内参主点坐标和校正映射不再匹配。
